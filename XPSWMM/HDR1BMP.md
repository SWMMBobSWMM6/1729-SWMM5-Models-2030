# 💧 HDR1BMP.inp

![flow units](https://img.shields.io/badge/flow_units-CFS-blue) ![routing](https://img.shields.io/badge/routing-Dynwave-2ea44f) ![infiltration](https://img.shields.io/badge/infiltration-Horton-9cf) ![nodes](https://img.shields.io/badge/nodes-18-informational) ![links](https://img.shields.io/badge/links-17-informational) ![subcatchments](https://img.shields.io/badge/subcatchments-0-informational) ![status](https://img.shields.io/badge/status-no_report-lightgrey)

> [!NOTE]
> No `HDR1BMP.rpt` found next to this model -- it may not have been run yet.

**Title:** SWMM  simulation of High Density Residential (HDR)  
**Location:** `XPSWMM/HDR1BMP.inp`

## ⚙️ Configuration

| Setting | Value |
|---|---|
| Flow units | CFS |
| Infiltration | HORTON |
| Flow routing | DYNWAVE |
| Routing step | 00:00:20 s |

## 🔧 Hydraulics

| Nodes | Count | Links | Count |
|---|--:|---|--:|
| Junctions | 18 | Conduits | 17 |
| Outfalls | 0 | Pumps | 0 |
| Storage | 0 | Orifices | 0 |
| Dividers | 0 | Weirs | 0 |
| **Total nodes** | **18** | Outlets | 0 |
|  |  | **Total links** | **17** |

> Cross-sections: **17**

---
<sub>📄 Auto-generated 2026-06-24 • summary of `HDR1BMP.inp`</sub>

# 💧 IMC1_SW.inp

![flow units](https://img.shields.io/badge/flow_units-CFS-blue) ![routing](https://img.shields.io/badge/routing-Dynwave-2ea44f) ![infiltration](https://img.shields.io/badge/infiltration-Horton-9cf) ![nodes](https://img.shields.io/badge/nodes-223-informational) ![links](https://img.shields.io/badge/links-222-informational) ![subcatchments](https://img.shields.io/badge/subcatchments-0-informational) ![status](https://img.shields.io/badge/status-no_report-lightgrey)

> [!NOTE]
> No `IMC1_SW.rpt` found next to this model -- it may not have been run yet.

**Title:** Indian Mill Creek Test Run  
**Location:** `XPSWMM/IMC1_SW.inp`

## ⚙️ Configuration

| Setting | Value |
|---|---|
| Flow units | CFS |
| Infiltration | HORTON |
| Flow routing | DYNWAVE |

## 🔧 Hydraulics

| Nodes | Count | Links | Count |
|---|--:|---|--:|
| Junctions | 223 | Conduits | 222 |
| Outfalls | 0 | Pumps | 0 |
| Storage | 0 | Orifices | 0 |
| Dividers | 0 | Weirs | 0 |
| **Total nodes** | **223** | Outlets | 0 |
|  |  | **Total links** | **222** |

> Cross-sections: **222**

---
<sub>📄 Auto-generated 2026-06-24 • summary of `IMC1_SW.inp`</sub>

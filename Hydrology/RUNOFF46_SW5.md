# 💧 RUNOFF46_SW5.inp

![flow units](https://img.shields.io/badge/flow_units-CFS-blue) ![routing](https://img.shields.io/badge/routing-Dynwave-2ea44f) ![infiltration](https://img.shields.io/badge/infiltration-Green__Ampt-9cf) ![nodes](https://img.shields.io/badge/nodes-5-informational) ![links](https://img.shields.io/badge/links-4-informational) ![subcatchments](https://img.shields.io/badge/subcatchments-4-informational) ![status](https://img.shields.io/badge/status-1_error-red)

> [!CAUTION]
> **Run failed** -- 1 fatal error(s) in `RUNOFF46_SW5.rpt`.<br>ERROR 323: cannot open runoff interface file D:\SWMMandSoftware\swmm_crada_files\Other SWMM5 Files\RUNOFF46.INT.

**Title:** SW 1 0 9A1 'BROWARD COUNTY Multi FAMILY SITE - Runoff example # 46'A2 'RUNO  
**Location:** `Hydrology/RUNOFF46_SW5.inp`

## ⚙️ Configuration

| Setting | Value |
|---|---|
| Flow units | CFS |
| Infiltration | GREEN_AMPT |
| Flow routing | DYNWAVE |
| Simulation start | 06/01/1977 15:55:00 |
| Simulation end | 06/01/1977 18:50:00 |
| Duration | 2 hr 55 min |
| Report step | 00:15:00 |
| Routing step | 0:00:30 s |

## 💧 Hydrology

| Item | Count |
|---|--:|
| Subcatchments | 4 |
| Total area (ac) | 14.74 |
| Raingages | 1 |

## 🔧 Hydraulics

| Nodes | Count | Links | Count |
|---|--:|---|--:|
| Junctions | 3 | Conduits | 4 |
| Outfalls | 1 | Pumps | 0 |
| Storage | 1 | Orifices | 0 |
| Dividers | 0 | Weirs | 0 |
| **Total nodes** | **5** | Outlets | 0 |
|  |  | **Total links** | **4** |

> Cross-sections: **4**

## ⚗️ Water Quality

Pollutants: **6** &nbsp;|&nbsp; Land uses: **1** &nbsp;|&nbsp; Treatment nodes: **6**

## 📈 Forcing & Data

| Item | Count |
|---|--:|
| Time series | 1 |

---
<sub>📄 Auto-generated 2026-06-24 • summary of `RUNOFF46_SW5.inp`</sub>

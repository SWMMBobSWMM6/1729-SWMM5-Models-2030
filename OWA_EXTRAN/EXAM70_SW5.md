# 💧 EXAM70_SW5.inp

![flow units](https://img.shields.io/badge/flow_units-CFS-blue) ![routing](https://img.shields.io/badge/routing-Dynwave-2ea44f) ![infiltration](https://img.shields.io/badge/infiltration-Horton-9cf) ![nodes](https://img.shields.io/badge/nodes-4-informational) ![links](https://img.shields.io/badge/links-3-informational) ![subcatchments](https://img.shields.io/badge/subcatchments-0-informational) ![status](https://img.shields.io/badge/status-2_errors-red)

> [!CAUTION]
> **Run failed** -- 2 fatal error(s) in `EXAM70_SW5.rpt`.<br>ERROR 141: Outfall 301 has more than 1 inlet link or an outlet link.; ERROR 141: Outfall 201 has more than 1 inlet link or an outlet link.

**Title:** 'EXTRAN USER''S MANUAL EXAMPLE PROBLEM 70 - ISOL = 0 SOLUTION ' ;'CIRCULAR PIPE COMPARISON WITH RUNOFF AND TRANSPORT BLOCKS' ;  
**Location:** `OWA_EXTRAN/EXAM70_SW5.INP`

## ⚙️ Configuration

| Setting | Value |
|---|---|
| Flow units | CFS |
| Infiltration | HORTON |
| Flow routing | DYNWAVE |
| Simulation start | 01/01/1988 00:00:00 |
| Simulation end | 01/01/1988 03:00:00 |
| Duration | 3 hr |
| Report step | 00:15:00 |
| Routing step | 0:00:10 s |

## 🔧 Hydraulics

| Nodes | Count | Links | Count |
|---|--:|---|--:|
| Junctions | 0 | Conduits | 3 |
| Outfalls | 4 | Pumps | 0 |
| Storage | 0 | Orifices | 0 |
| Dividers | 0 | Weirs | 0 |
| **Total nodes** | **4** | Outlets | 0 |
|  |  | **Total links** | **3** |

> Cross-sections: **3**

---
<sub>📄 Auto-generated 2026-06-24 • summary of `EXAM70_SW5.inp`</sub>

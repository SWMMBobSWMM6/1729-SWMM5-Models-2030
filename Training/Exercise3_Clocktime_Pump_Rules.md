# 💧 Exercise3_Clocktime_Pump_Rules.inp

![flow units](https://img.shields.io/badge/flow_units-LPS-blue) ![routing](https://img.shields.io/badge/routing-Dynwave-2ea44f) ![infiltration](https://img.shields.io/badge/infiltration-Horton-9cf) ![nodes](https://img.shields.io/badge/nodes-61-informational) ![links](https://img.shields.io/badge/links-61-informational) ![subcatchments](https://img.shields.io/badge/subcatchments-0-informational) ![status](https://img.shields.io/badge/status-no_report-lightgrey)

> [!NOTE]
> No `Exercise3_Clocktime_Pump_Rules.rpt` found next to this model -- it may not have been run yet.

**Title:** Scenario Run :  ONEMINUTE  
**Location:** `Training/Exercise3_Clocktime_Pump_Rules.inp`

## ⚙️ Configuration

| Setting | Value |
|---|---|
| Flow units | LPS |
| Infiltration | HORTON |
| Flow routing | DYNWAVE |
| Simulation start | 01/01/1988 00:00:00 |
| Simulation end | 01/01/1988 06:00:00 |
| Duration | 6 hr |
| Report step | 00:01:00 |
| Routing step | 0:00:10 s |

## 🔧 Hydraulics

| Nodes | Count | Links | Count |
|---|--:|---|--:|
| Junctions | 51 | Conduits | 54 |
| Outfalls | 6 | Pumps | 4 |
| Storage | 4 | Orifices | 2 |
| Dividers | 0 | Weirs | 1 |
| **Total nodes** | **61** | Outlets | 0 |
|  |  | **Total links** | **61** |

> Cross-sections: **57**

## 🎛️ Control Rules

Real-time control rules: **3**

## ⚗️ Water Quality

Pollutants: **1** &nbsp;|&nbsp; Land uses: **0**

## 📈 Forcing & Data

| Item | Count |
|---|--:|
| Direct inflows | 19 |
| Dry-weather inflows | 3 |
| Time series | 20 |
| Curves | 13 |

---
<sub>📄 Auto-generated 2026-06-24 • summary of `Exercise3_Clocktime_Pump_Rules.inp`</sub>
